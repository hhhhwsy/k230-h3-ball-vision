...
2026 年全国大学生电子设计竞赛 H 题（车载平衡滚球）视觉子系统，个人技术练习与代码整理项目。
在立创·庐山派 K230 上，用**一个摄像头**同时完成两件互相冲突的事：给主控喂实时球位、给上位机推实时画面。
...
本仓库代码在 **AI 编程助手辅助下完成**。本人负责的部分是：需求拆解、方案取舍、实机联调、参数整定、串口协议对接、现场调试与比赛期间的应急处置；**具体的代码实现有相当一部分由 AI 生成**。本人对整体架构、数据流和各个参数的含义是掌握的，但对 CanMV/MicroPython 底层驱动的部分 API 细节仍需查阅文档。
...
> **为什么实拍里没有识别框？**
> 图传录制的是 **CH0 干净画面**，这是刻意的设计（见第六节「线程模型」）。识别框、ROI 边界、凹槽中心线只绘制在 **CH1 视觉帧**上、显示于本地 ST7701 屏，**因此不出现在图传录像里**。
> 要看识别效果，需要单独录制本地屏，或把 `DISPLAY_TO_IDE` 改为 `True` 后从 CanMV IDE 预览窗口录制（详见 `发布说明.md`）。
...
>
> 30 fps 这个数**有实测依据**：比赛期间用 OBS 从 RTSP 拉流录制了 5 段视频（共约 17 分钟），`ffprobe` 显示全部为 **H.264 / 1280×720 / 30 fps**。而 25 Hz 由代码里的发送间隔决定（`UART_SEND_INTERVAL_MS = 40`），属于设计值，未做长时间统计。
>
...
单帧几何匹配在赛场不够用（刻度、接缝、车体反光都会形成合格候选），因此加了三态跟踪：
...
**脱机部署**：联调通过后重命名为 `/sdcard/main.py`，断电重启由 `__main__` 自启，不依赖电脑与 IDE。正式比赛采用「上电只启动一次」，可显著降低重复初始化造成的阻塞概率。
...
确认识别稳定后，正式比赛可关闭本地显示与调试绘制，把时间全部留给视觉：
...
| 现场可调性 | 阈值、面积、ROI、标定点都能在赛场直接改，改完立刻见效；模型要重采重训，赛前时间不够 | 强（工程现实） |
...
- **现有的图传录像无法作为"识别效果"的证据**：录的是 CH0 干净画面，识别框只绘在本地屏的 CH1 帧上。要证明识别有效，需要补录本地屏或 IDE 预览画面
...
热点建议：只用 2.4 GHz、名称与密码先用纯英文数字、关闭连接数上限、电脑与 K230 连同一局域网、比赛期间固定不变。**不要把真实密码提交到仓库。**
