...
这是竞赛项目初期并行评估的深度学习路线。**从板载采图到量化转换的链路是通的**，但最终没有选它作为比赛方案——当时的取舍考虑见主仓库 README 的「关于技术路线的说明」一节。
...
COCO 预训练权重里没有「小钢球」这个类别。其中相近的 `sports ball` 在赛场综合光照与金属反光下误检、漏检严重，无法作为闭环控制的输入。所以必须用**真实赛场光照、背景、距离和相机角度**自行采集并标注。
...
> **量化校准集的图片必须来自最终摄像头与真实赛场环境**，否则反光金属球在 PTQ 后漏检会非常严重。
...
**这个方案没有被选为比赛方案，因此也没有走完最后的精度验证。** 诚实记录如下：
...
| 问题 | 原因 | 解决 |
|---|---|---|
| 量化后金属球漏检严重 | 钢球强反光，PTQ 校准图分布与赛场差异大 | 校准集改用赛场同款摄像头与光照下的图片，必要时补采强反光样本重训 |
